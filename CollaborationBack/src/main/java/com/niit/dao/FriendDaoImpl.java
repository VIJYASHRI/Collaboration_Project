...
public class FriendDaoImpl implements FriendDao {
	...
	public List<User> getSuggestedUsers(User user) {
		...
		SQLQuery query = session.createSQLQuery(
				"select * from User where username in (select username from USER where username!=? minus (select from_id from friend where to_id=?"
	...
	/*	Criteria criteria=session.createCriteria(Friend.class);
		...
		criteria.add(Restrictions.eq("to", username));
